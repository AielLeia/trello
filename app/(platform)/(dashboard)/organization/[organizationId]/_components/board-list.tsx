  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
        {boards.map((board) => (
          <Link
            href={route('/board/[boardId]', { params: { boardId: board.id } })}
            key={board.id}
            className="group relative aspect-video bg-no-repeat bg-center bg-sky-700 bg-cover rounded-sm h-full w-full p-2"
            style={{
...
export const BoardListSkeleton = () => {
  return (
    <div className="space-y-4">
      <div className="flex items-center font-semibold text-neutral-700 text-lg">
        <Skeleton className="h-6 w-[732px]" />
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
